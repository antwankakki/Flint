...
// The compiler has checks enabled to spit out warnings when variables/objects are
// declared and not used and has switches enabled to treat warnings at errors.
...
#define UNUSED(x)                                                                                                          \
  debugPrint(LogType::INFO, __DEBUG__ || __RELEASE__, "Unused parameter (%s) found at %s:%s", #x, __FILE__, __FUNCTION__); \
  (void)x;
...
#define SAFE_FREE(x)                                                                 \
  if (x)                                                                             \
  {                                                                                  \
    debugPrint(LogType::INFO, __DEBUG__ || __RELEASE__, "Safely freeing %s", #x);    \
    free(x);                                                                         \
    x = NULL;                                                                        \
  }                                                                                  \
  else                                                                               \
  {                                                                                  \
    debugPrint(LogType::INFO, __DEBUG__ || __RELEASE__, "%s is a free pointer", #x); \
  };
